...
def enclose_script(script):
    ...
    template_start = script.find(
        "############################# Template Variables #############################"
    )
    template_end = script.find(
        "############################## Expect Variables ##############################"
    )
...
def main():
    if len(sys.argv) < 2:
        print(
            "Usage: python3 expect_to_python.py <expect_script.exp>",
            file=sys.stderr,
        )
        print(
        ...
        print(
            "Output can be copied directly into Python source code.",
            file=sys.stderr,
        )
        sys.exit(1)
