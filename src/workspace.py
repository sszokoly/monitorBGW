...
from layouts import LAYOUTS, RTP_LAYOUT, Layout, iter_attrs, COLORS
from bgw import BGW
from storage import MemoryStorage
from rtpparser import parse_rtpstat
from utils import logger, config
from datetime import datetime
from async_loop import discovery, schedule_http_server, schedule_queries, GWs, BGWs, RTPs, PCAPs, Capture
from async_loop import startup_async_loop, schedule_task, request_shutdown, finalize_loop_if_idle, tick_async_loop
from filter_parser import filter_validator, FILTER_GROUPs, FILTER_MENUs, update_filter
############################## BEGIN VARIABLES ################################
...
def capture_upload(ws):
    ...
    filename = f"{datetime.now().strftime('%Y%m%d_%H%M%S')}_{bgw.gw_number}.cap"
