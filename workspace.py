...
from typing import Any, Dict, List, Tuple, Optional, Callable, TYPE_CHECKING, Sequence, Iterable
...
from screens import LAYOUTS, RTP_LAYOUT, Layout, iter_attrs, COLORS
from bgw import BGW
...
from async_loop import startup_async_loop, schedule_task, request_shutdown, finalize_loop_if_idle, tick_async_loop
from filter_parser import filter_parser, filter_validator, FILTER_GROUPs, FILTER_MENUs, update_filter
############################## BEGIN VARIABLES ################################
...
class Workspace(object):
    ...
    def draw_bodywin(self, dim: bool=False, xoffset: int=1) -> None:
        ...
        visible = self.storage[self.storage_cursor:end_idx]
        colorb = self.color_border | curses.A_DIM if dim else self.color_border
...
def discovery_start(ws):
    ...
    def discovery_done_callback(fut):
        """Set Button back to OFF state if discovery completed"""
        if fut.cancelled():
            return
        curses.flushinp()
        # Simulate user stopping discovery
...
def polling_start(ws):
    ...
    def process_item_callback():
        ...
        if polling_workspace:
            if aws.panel.hidden():
                rtpdetails = aws.storage.get(ws.storage_cursor + ws.body_posy)
...
def show_rtp(ws):
    rtpdetails = ws.storage.get(ws.storage_cursor + ws.body_posy)
